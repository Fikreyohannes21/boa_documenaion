...
2. ```ES_Desktop ``` Installation
    ...
    ![step 5](/img/upgrade3.png "step 5")
      ...
      - Please enter ```java jdk installation location``` for your windows pc and click ```next``` 
      ...
      - To use ```ES_Desktop``` next time the ```RunDektop.bat``` file or the ```DesktopLauncher.exe``` can be used  